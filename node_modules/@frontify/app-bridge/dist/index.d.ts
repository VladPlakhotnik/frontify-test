...
export declare class AppBridgePlatformApp implements IAppBridgePlatformApp {
    private apiMessageBus;
    private stateMessageBus;
    private initialized;
    ...
    subscribe<EventName extends keyof PlatformAppEvent>(eventName: EventNameParameter<EventName, PlatformAppEvent>, callback: EventCallbackParameter<EventName, PlatformAppEvent>): EventUnsubscribeFunction;
    callSubscribedTopic<EventName extends keyof PlatformAppEvent>(eventName: EventNameParameter<EventName, PlatformAppEvent>, callbackParameters: PlatformAppEvent[EventName]): void;
}
...
export declare enum AssetBulkDownloadState {
    Init = "init",
    Started = "started",
    Pending = "pending",
    Ready = "ready",
...
export declare type BlockApiMethod = ApiMethodNameValidator<Pick<ApiMethodRegistry, 'getAssetBulkDownloadToken' | 'getCurrentUser'>>;

export declare type BlockCommand = CommandNameValidator<Pick<CommandRegistry, 'closeAssetChooser' | 'closeTemplateChooser' | 'downloadAsset' | 'openAssetChooser' | 'openAssetViewer' | 'openTemplateChooser' | 'openNewPublication'>>;
...
export declare type CommandVerb = 'open' | 'close' | 'navigate' | 'download';
...
export declare const createAsset: (payload: CreateAssetPayload) => {
    name: 'createAsset';
    payload: CreateAssetPayload;
};
...
declare type DownloadAsset = Asset;

export declare const downloadAsset: (asset: CommandRegistry['downloadAsset']) => DispatchHandlerParameter<'downloadAsset', CommandRegistry>;
...
export declare const getAppBridgePlatformAppStub: () => SinonStubbedInstance<IAppBridgePlatformApp>;

export declare type getAppBridgePlatformAppStubProps = {};

export declare const getAppBridgeThemeStub: ({ editorState, brandId, portalId, projectId, pageTemplateSettings, pageTemplateAssets, themeSettings, themeAssets, language, openAssetChooser, closeAssetChooser, }?: getAppBridgeThemeStubProps) => SinonStubbedInstance<AppBridgeTheme>;
...
export declare type GetAssetBulkDownloadTokenPayload = {
    documentBlockId: number;
    settingIds?: string[];
};

export declare type GetAssetBulkDownloadTokenResponse = {
    assetBulkDownloadToken: string;
};

export declare const getAssetResourceInformation: (payload: GetAssetResourceInformationPayload) => {
    name: 'getAssetResourceInformation';
    payload: GetAssetResourceInformationPayload;
};

export declare type GetAssetResourceInformationPayload = {
    assetId: string;
};

export declare type GetAssetResourceInformationResponse = {
    type: string;
    id: string;
    title?: string;
    previewUrl?: string;
...
export declare const getCurrentUser: (payload: GetCurrentUserPayload) => {
    name: string;
    payload: void;
};

export declare type GetCurrentUserPayload = void;

export declare type GetCurrentUserResponse = {
    id: string;
    name?: string | null;
    avatar?: string | null;
    email: string;
...
export declare interface IAppBridgePlatformApp<State extends PlatformAppState = PlatformAppState, Context extends PlatformAppContext = PlatformAppContext, Event extends PlatformAppEvent = PlatformAppEvent> extends AppBridge<PlatformAppApiMethod, PlatformAppCommand, State, Context, Event> {
    api<ApiMethodName extends keyof PlatformAppApiMethod>(apiHandler: ApiHandlerParameter<ApiMethodName, PlatformAppApiMethod>): ApiReturn<ApiMethodName, PlatformAppApiMethod>;
    dispatch<CommandName extends keyof PlatformAppCommand>(dispatchHandler: DispatchHandlerParameter<CommandName, PlatformAppCommand>): Promise<void>;
    state(): StateReturn<State, void>;
    state<Key extends keyof State>(key: Key): StateReturn<State, Key>;
...
export declare const openNewPublication: (payload: CommandRegistry['openNewPublication']) => DispatchHandlerParameter<'openNewPublication', CommandRegistry>;

export declare type OpenNewPublicationPayload = {
    template: Template;
};
...
export declare type PlatformAppApiMethod = ApiMethodNameValidator<Pick<ApiMethodRegistry, 'getCurrentUser' | 'getAssetResourceInformation' | 'createAsset'>>;

export declare type PlatformAppCommand = CommandNameValidator<Pick<PlatformAppCommandRegistry, 'openConnection'>>;

export declare type PlatformAppCommandRegistry = CommandNameValidator<{
    openConnection: void;
}>;

export declare type PlatformAppContext = AppBaseProps & ({
    assetId: string;
    brandId: string;
    domain: string;
    parentId: string;
...
export declare const useAssetBulkDownload: (appBridge: AppBridgeBlock) => {
    generateBulkDownload: (settingIds?: string[]) => Promise<void>;
...
export declare const usePageTemplateSettings: <TPageTemplateSettings = Record<string, unknown>>(appBridge: AppBridgeTheme, template: 'cover' | 'documentPage' | 'library', documentOrDocumentPageId?: number) => {
    pageTemplateSettings: Nullable<TPageTemplateSettings & Record<string, unknown>>;
    customizedPageTemplateSettingsKeys: string[];
    updatePageTemplateSettings: (pageTemplateSettingsUpdate: Partial<TPageTemplateSettings>) => Promise<void>;
    isLoading: boolean;
};

export declare const usePlatformAppBridge: () => AppBridgePlatformApp | undefined;
...
export declare const useTemplateAssets: (appBridge: AppBridgeTheme, template?: 'documentPage' | 'library' | 'cover', documentId?: number, documentPageId?: number) => {
    templateAssets: Record<string, Asset[]>;
    customizedTemplateAssetsKeys: string[];
    addAssetIdsToKey: (key: string, assetIds: number[]) => Promise<void>;
    deleteAssetIdsFromKey: (key: string, assetIds: number[]) => Promise<void>;
    updateAssetIdsFromKey: (key: string, newAssetIds: number[]) => Promise<void>;
};
